...
| NL query layer | Deterministic intent classification (keyword/regex) → entity resolution (`rapidfuzz`) → dispatch to the matching query template → string-templated answer. Explicit non-answer for anything outside the 7 supported questions (FR13). | `src/nlquery/` |
...
3. The NL query route (FR11/FR13) first classifies intent and resolves named entities via `src/nlquery/`, then dispatches to the *same* query templates used by the structured routes — the NL layer is a thin deterministic front-end over the structured queries, not a separate answer path. If intent classification or entity resolution fails, it returns the explicit non-answer (FR13) rather than falling through to a guess.
...
5. Every rendered answer includes its evidence (source system(s), relationship types, and — for paths — the full path) per FR12, sourced from the same query result, never re-derived separately.
...
| `answer_generator` | `src/nlquery/answering.py::AnswerGenerator` (Protocol) | Deterministic string templates over query results (`ADR-0004`) | MVP2 Graph RAG: LLM-backed grounded generation over the same retrieved subgraph, behind the identical interface — the query/retrieval layer underneath does not change |
